...
public class ViewMauSac extends javax.swing.JFrame {
    ...
    private Long idMauSac = null;

    public ViewMauSac() {
        initComponents();
        dtm = (DefaultTableModel) tbMauSac.getModel();
        listMS = mauSacDao.findAll();
        showTable(mauSacDao.findAll());
        showIndex(0);
    }

    private void showTable(List<MauSac> list) {
        dtm.setRowCount(0);
        int index = 0;
    ...
    private MauSac showIndex(int index) {
        listMS = mauSacDao.findAll();
        MauSac ms = listMS.get(index);
        txtTen.setText(ms.getTen());
        if (ms.getTrangThai().equalsIgnoreCase(trangThaiBienThe.HOAT_DONG.value)) {
    ...
    public MauSac data() {
        ...
        MauSac ms = new MauSac(Long.MIN_VALUE, ten, trangThai);
        return ms;
    }
    ...
    private void tbMauSacMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbMauSacMouseClicked
        ...
        showIndex(index);
        idMauSac = showIndex(index).getId();
        System.out.println("id ms : " + idMauSac);
    }//GEN-LAST:event_tbMauSacMouseClicked

    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        if (data() != null) {
            MauSac mauSacAdd = data();
            if (mauSacAdd != null) {
                if (!mauSacDao.existsByName(mauSacAdd.getTen())) // Tiến hành sửa
                {
                    JOptionPane.showMessageDialog(this, mauSacDao.create(mauSacAdd));
                    // Cập nhật lại bảng sau khi sửa
                    showTable(mauSacDao.findAll());
                } else {
                    JOptionPane.showMessageDialog(this, "Tên : " + mauSacAdd.getTen() + " => Đã Tồn Tại");
                }
            }
        } else {
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        if (data() != null) {
            ...
            msUpdate.setId(idMauSac);  // Giả sử `idMauSac` lấy từ dòng đã chọn
            if (msUpdate != null) {
                int chon = JOptionPane.showConfirmDialog(this, "Bạn có muốn sửa không?");
                if (chon == JOptionPane.OK_OPTION) {
                    if (!mauSacDao.existsByName(msUpdate.getTen())) // Tiến hành sửa
                    {
                        JOptionPane.showMessageDialog(this, mauSacDao.update(msUpdate));
                        // Cập nhật lại bảng sau khi sửa
                        showTable(mauSacDao.findAll());
                    } else {
                        JOptionPane.showMessageDialog(this, "Tên : " + msUpdate.getTen() + " => Đã Tồn Tại");
                    }
